#!/usr/bin/env python3
"""Train a small causal TCN on subject-disjoint pose windows."""
...
def main() -> int:
    ...
    if window_index.get("sequence_contract_version") != "observed_only_10hz_v2":
        raise ValueError("training requires observed_only_10hz_v2 windows")
    ...
    model = FallTCN(train_x.shape[2]).to(device)
    ...
    print(json.dumps({key: report[key] for key in ("model", "device", "parameter_count", "epochs_completed", "best_val_loss", "validation", "test")}, ensure_ascii=False, indent=2))
    return 0
