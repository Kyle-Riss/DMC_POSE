#!/usr/bin/env python3
"""Train a shadow-only Swin3D delta probe with held-out positive-session CV."""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=PROJECT_ROOT
        / "runs/video_verifier/swin3d_b_reviewed_group_cv_v1_20260828",
    )
    ...
    staged_train_x, staged_train_y = load_pairs(args.staged, "train")
    staged_val_x, staged_val_y = load_pairs(args.staged, "val")
    staged_test_x, staged_test_y = load_pairs(args.staged, "test")

    positive_groups = sorted(set(groups[y_reviewed == 1]))
    if len(positive_groups) != 2:
        raise ValueError(
            f"expected exactly two positive session groups, got {positive_groups}"
        )
    ...
    for c in C_VALUES:
        ...
        for fold, positive_group in enumerate(positive_groups):
            test_mask = (groups == positive_group) | np.asarray(
                [
                    y == 0 and negative_fold.get(group) == fold
                    for y, group in zip(y_reviewed, groups)
                ],
                dtype=bool,
            )
            train_mask = ~test_mask
            train_x = np.concatenate([staged_train_x, x_reviewed[train_mask]], axis=0)
            train_y = np.concatenate([staged_train_y, y_reviewed[train_mask]], axis=0)
            scaler, model = fit(train_x, train_y, c)
            test_probability = probability(scaler, model, x_reviewed[test_mask])
            for event_id, target, score in zip(
                event_ids[test_mask], y_reviewed[test_mask], test_probability
            ):
                held_rows.append(
                    {
                        "fold": fold,
            ...
            fold_reports.append(
                {
                    "fold": fold,
                    "held_positive_group": positive_group,
                    "events": int(test_mask.sum()),
                    "positive": int(y_reviewed[test_mask].sum()),
                    "negative": int(test_mask.sum() - y_reviewed[test_mask].sum()),
                }
            )
    ...
    final_x = np.concatenate([staged_train_x, x_reviewed], axis=0)
    final_y = np.concatenate([staged_train_y, y_reviewed], axis=0)
    scaler, model = fit(final_x, final_y, selected_c)
    staged_probability = {
        "validation": probability(scaler, model, staged_val_x),
        "test": probability(scaler, model, staged_test_x),
    }
    ...
    report = {
        "schema_version": "dmc_swin3d_reviewed_group_cv_probe_v1",
        ...
        "staged_validation": metrics(staged_val_y, staged_probability["validation"], threshold),
        "staged_test": metrics(staged_test_y, staged_probability["test"], threshold),
        "search": search,
        ...
        "warnings": [
            ...
            "events came from an automatic-candidate selection process",
            "final model uses all reviewed events and requires a new untouched session test",
        ],
    }
    ...
    print(json.dumps({
        ...
        "promotion_eligible": False,
        "output": str((args.out_dir / "delta_probe.npz").resolve()),
